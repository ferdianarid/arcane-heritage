...
export const QuizCategoryCard = ({ name, href }: QuizCategoryCardProps) => {
  return (
    <Link href={href as string} className="group">
      <Card className="flex relative overflow-hidden w-full flex-col items-center justify-center text-center bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 h-[300px] transition-colors duration-300 backdrop-blur-sm">
        <Image
          src={
            name.toLowerCase().includes("makanan")
              ? "/assets/images/foods/pempek.jpg"
              : "/assets/images/buildings/klenteng-sam-poo-kong.jpg"
          }
          fill
